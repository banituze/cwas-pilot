"""Code 128 barcodes (character set B) drawn as inline SVG for receipts, with no extra dependency.

PATTERNS is the published Code 128 symbol table (ISO/IEC 15417): values 0 to 102, the start codes A, B and C
(103, 104, 105) and the stop pattern (106). Each "1" is one dark module, each "0" one light module.
"""
...
def svg(text, height=52, cls="c128"):
    """A scannable barcode as inline SVG. Runs of dark modules become one rectangle; 10 light modules each side."""
    bits, quiet = modules(text), 10
    rects, i = [], 0
    while i < len(bits):
        if bits[i] == "1":
            j = i
            ...
            i += 1
    width = len(bits) + 2 * quiet
    return Markup(f'<svg class="{cls}" viewBox="0 0 {width} {height}" preserveAspectRatio="none" shape-rendering="crispEdges" '
                  f'role="img" aria-label="{escape(text)}">' + "".join(rects) + "</svg>")
